...
public class Main {
    ...
    private static final String WINDOW_TITLE = "GPU Renderer - Dragon Ray Tracing";
    private static final double TITLE_UPDATE_INTERVAL = 0.25;
    ...
    private void updateWindowTitle(float deltaTime) {
        ...
        if (fpsUpdateTimer < TITLE_UPDATE_INTERVAL) {
            return;
        }

        double fps = fpsFrameCounter / fpsUpdateTimer;
        window.setTitle(String.format("%s | FPS %.1f", WINDOW_TITLE, fps));
        fpsUpdateTimer = 0.0;
        fpsFrameCounter = 0;
    }
