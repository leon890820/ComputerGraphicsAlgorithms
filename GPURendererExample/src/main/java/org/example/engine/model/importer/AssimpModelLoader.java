package org.example.engine.model.importer;
...
import org.example.engine.model.ModelNode;
import org.example.engine.model.animation.ModelAnimationClip;
import org.example.engine.model.animation.ModelNodeAnimation;
...
import org.example.engine.model.skin.ModelBone;
...
public class AssimpModelLoader {
    ...
    public ModelAsset load(String path) {
        ...
        if (scene == null) {
            System.out.println("[AssimpModelLoader] load failed: " + path);
            System.out.println("[AssimpModelLoader] resolved path: " + resolvedPath);
            System.out.println("[AssimpModelLoader] " + aiGetErrorString());
        ...
        try {
            ...
            System.out.println("[AssimpModelLoader] loaded: " + resolvedPath);
            System.out.println("[AssimpModelLoader] nodes = " + asset.nodes.size()
    ...
    private void loadMeshes(AIScene scene, ModelAsset asset) {
    ...
    private void readSkin(AIMesh aiMesh, ModelAsset asset, String meshName, ModelPrimitive primitive) {
        ...
        System.out.println("[AssimpModelLoader] skin " + skin.name
                + " bones = " + skin.bones.size()
                + ", vertices = " + vertexCount);
    }

    private void addBoneInfluence(ModelPrimitive primitive, int vertexIndex, int boneIndex, float weight) {
    ...
    private void normalizeBoneWeights(ModelPrimitive primitive) {
    ...
    private void loadNodes(AIScene scene, ModelAsset asset) {
    ...
    private int readNodeRecursive(AINode aiNode, int parentIndex, ModelAsset asset) {
    ...
    private void loadAnimations(AIScene scene, ModelAsset asset) {
    ...
    private ModelNodeAnimation readNodeAnimation(AINodeAnim channel, ModelAsset asset) {
        ModelNodeAnimation out = new ModelNodeAnimation();
        out.nodeName = channel.mNodeName().dataString();
        Integer nodeIndex = asset.nodeNameToIndex.get(out.nodeName);
        if (nodeIndex != null) {
            out.nodeIndex = nodeIndex;
        } else {
            System.out.println("[AssimpModelLoader] animation channel node not found: " + out.nodeName);
        }
        ...
        if (positionKeys != null) {
            for (int i = 0; i < channel.mNumPositionKeys(); i++) {
                ...
                out.positionKeys.add(new ModelNodeAnimation.VectorKeyframe(
                        key.mTime(),
        ...
        if (rotationKeys != null) {
            for (int i = 0; i < channel.mNumRotationKeys(); i++) {
                ...
                var value = key.mValue();
                out.rotationKeys.add(new ModelNodeAnimation.QuaternionKeyframe(
                        key.mTime(),
        ...
        if (scaleKeys != null) {
            for (int i = 0; i < channel.mNumScalingKeys(); i++) {
                ...
                var value = key.mValue();
                out.scaleKeys.add(new ModelNodeAnimation.VectorKeyframe(
                        key.mTime(),
                        new Vector3(value.x(), value.y(), value.z())
                ));
            }
    ...
    private Texture loadMaterialTexture(AIScene scene, int materialIndex) {
        ...
        try {
            ...
            if (result != aiReturn_SUCCESS) {
                System.out.println("[AssimpModelLoader] no diffuse/baseColor texture for material_" + materialIndex);
                return null;
            }

            String path = texturePath.dataString();
            System.out.println("[AssimpModelLoader] material_" + materialIndex + " texture ref = " + path);
    ...
    private Texture loadEmbeddedTexture(AIScene scene, String ref) {
        ...
        if (textureIndex < 0 || textureIndex >= scene.mNumTextures()) {
            System.out.println("[AssimpModelLoader] invalid embedded texture ref: " + ref);
            return null;
        }
