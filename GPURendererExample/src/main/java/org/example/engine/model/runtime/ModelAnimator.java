package org.example.engine.model.runtime;
...
public class ModelAnimator {
    ...
    private ModelAnimationClip currentClip;
    ...
    public ModelAnimator(ModelAsset asset) {
        this.asset = asset;
        allocatePoseBuffers();
        playFirstAnimation();
    }
    ...
    public void play(String clipName) {
        ...
        for (ModelAnimationClip clip : asset.animations) {
    ...
    public void play(ModelAnimationClip clip) {
        currentClip = clip;
        currentTime = 0.0;
        ...
        if (currentClip != null) {
            System.out.println("[ModelAnimator] play animation: " + currentClip.name
    ...
    public ModelAnimationClip getCurrentClip() {
        return currentClip;
    }
    ...
    public ModelAnimator setLoop(boolean loop) {
        this.loop = loop;
        return this;
    }
    ...
    private void allocatePoseBuffers() {
        ...
        for (int skinIndex = 0; skinIndex < skinCount; skinIndex++) {
            ModelSkin skin = asset.skins.get(skinIndex);
    ...
    private void updatePose(double animationTime) {
        ...
        for (int i = 0; i < asset.nodes.size(); i++) {
            ModelNode node = asset.nodes.get(i);
            localMatrices[i] = node.localTransform;
        }

        if (currentClip != null) {
            for (ModelNodeAnimation channel : currentClip.channels) {
    ...
    private void updateSkinBoneMatrices() {
        ...
        for (int skinIndex = 0; skinIndex < asset.skins.size(); skinIndex++) {
            ModelSkin skin = asset.skins.get(skinIndex);
            for (int boneIndex = 0; boneIndex < skin.bones.size(); boneIndex++) {
                ModelBone bone = skin.bones.get(boneIndex);
    ...
    private Vector3 sampleVector(java.util.ArrayList<ModelNodeAnimation.VectorKeyframe> keys, double time, Vector3 fallback) {
        ...
        for (int i = 0; i < keys.size() - 1; i++) {
            ModelNodeAnimation.VectorKeyframe a = keys.get(i);
            ModelNodeAnimation.VectorKeyframe b = keys.get(i + 1);
            if (time <= b.time) {
    ...
    private Vector4 sampleQuaternion(java.util.ArrayList<ModelNodeAnimation.QuaternionKeyframe> keys, double time, Vector4 fallback) {
        if (keys == null || keys.isEmpty()) {
            return fallback;
        }
        ...
        for (int i = 0; i < keys.size() - 1; i++) {
            ModelNodeAnimation.QuaternionKeyframe a = keys.get(i);
            ModelNodeAnimation.QuaternionKeyframe b = keys.get(i + 1);
            if (time <= b.time) {
                float t = interpolationFactor(a.time, b.time, time);
